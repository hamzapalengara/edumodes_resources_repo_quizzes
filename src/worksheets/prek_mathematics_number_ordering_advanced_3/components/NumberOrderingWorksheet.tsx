...
const LEVELS = [
  { numbers: [12, 10, 11, 13], label: "Level 1 (10-13)" },
  { numbers: [13, 11, 14, 10, 12], label: "Level 2 (10-14)" },
  { numbers: [12, 15, 10, 14, 11, 13], label: "Level 3 (10-15)" }
];
...
const NumberOrderingWorksheet: React.FC = () => {
  ...
  const handleDragEnd = useCallback((
    ...
    const correctOrder = Array.from({ length: levelNumbers.length }, (_, i) => i + 10);
    ...
    >
      {({ score, markCorrect, markAttempted }) => (
        <div className="min-h-screen bg-gradient-to-b from-indigo-100 to-purple-100">
          ...
          <div className="bg-indigo-100/80 p-4 shadow-md">
            ...
            <main className="px-0 md:px-4 py-8">
              <div className="max-w-2xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                  <h1 className="text-2xl md:text-3xl font-bold text-indigo-600 text-center mb-6">
                    Put the Numbers in Order! 🔢
                  </h1>

                  {/* Level Indicator */}
                  <div className="text-center mb-4">
                    <span className="text-sm md:text-base text-indigo-600 font-semibold bg-indigo-100 rounded-full px-4 py-1 inline-block">
                  ...
                  <div className="text-center mb-6">
                    <p className="text-sm md:text-base text-gray-600 bg-indigo-50 rounded-full px-4 py-2 inline-block">
                  ...
                  <div className="flex justify-center gap-4 mb-8 flex-wrap">
                    {LEVELS[currentLevel].numbers.map((num) => (
                      ...
                      >
                        <span className="font-bold text-3xl md:text-4xl text-indigo-600 pointer-events-none select-none">
                          {num}
                        </span>
                      </motion.div>
                    ))}
                  ...
                  <div className="flex justify-center gap-4 flex-wrap">
                    {Array.from({ length: LEVELS[currentLevel].numbers.length }, (_, i) => i + 10).map((_, index) => (
                      <motion.div
                        key={index}
                        ref={el => targetRefs.current[index] = el}
                        className={`w-16 h-16 rounded-xl shadow-md flex items-center justify-center
                          ${completedPositions[index] ? 'bg-green-100 border-green-400' : 'bg-indigo-50 border-indigo-300'}
                      ...
                      >
                        {placedNumbers[index] !== null && (
                          <span className="font-bold text-3xl md:text-4xl text-indigo-600 select-none">
                            {placedNumbers[index]}
                          </span>
                        )}
                      </motion.div>
                  ...
                  {gameCompleted && (
                    <div className="mt-6 text-center">
                      <button
                        onClick={resetGame}
                        className="px-6 py-2 bg-indigo-500 text-white rounded-full hover:bg-indigo-600 active:bg-indigo-700 transition-colors"
                      >
                        Play Again 🔄
                      </button>
                    </div>
