...
const WORD_LIST = [
  // -et words
  { word: 'PET', pattern: '-et', hint: 'Animals in nature', emoji: '🐾' },
  { word: 'NET', pattern: '-et', hint: 'Used to catch fish', emoji: '🎣' },
  { word: 'JET', pattern: '-et', hint: 'Flies in the sky', emoji: '✈️' },
  
  // -in words
  { word: 'BIN', pattern: '-in', hint: 'Used to keep nature clean', emoji: '🗑️' },
  { word: 'PIN', pattern: '-in', hint: 'Small sharp object', emoji: '📌' },
  { word: 'TIN', pattern: '-in', hint: 'Used for storing food', emoji: '🥫' },
  
  // -ug words
  { word: 'BUG', pattern: '-ug', hint: 'Insects in nature', emoji: '🐛' },
  { word: 'MUG', pattern: '-ug', hint: 'Used for drinking water', emoji: '☕' },
  { word: 'JUG', pattern: '-ug', hint: 'Used to store water', emoji: '🏺' }
];
...
const PetRhymingWorksheet: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasPlayedIntro && words.length > 0) {
      speak("Welcome to our gentle pet-themed rhyming adventure! Let's practice spelling together. Click the speaker to hear each word.");
  ...
  const handleLetterClick = (letter: string, { markCorrect, markAttempted, markIncorrect }: any) => {
    ...
    if (!isCorrectLetter) {
      markIncorrect();
      speak("Take your time, try again!");
      return;
    }
      ...
      if (isWordCorrect) {
        ...
        setTimeout(() => {
          ...
          if (currentWordIndex < words.length - 1) {
            ...
            setIsComplete(true);
            speak("Wonderful! You've completed all the rhyming words!");
          }
        }, 2000);
      } else {
        markIncorrect();
            ...
            <TouchContainer>
              <div className="px-0 md:px-4">
                <div className="max-w-4xl mx-auto">
                  {/* Score Display */}
                  <div className="bg-[#374151]/40 backdrop-blur-sm rounded-2xl p-4 mb-6 border border-[#4B5563]">
                    <ScoreDisplay 
                      score={score}
                      totalQuestions={90}
                    />
                  ...
                  <div className="bg-[#374151]/40 backdrop-blur-sm rounded-2xl p-6 border border-[#4B5563]">
                    {!isComplete ? (
                      <>
                        ...
                        {/* Hint Display */}
                        {showHint && (
                          <div className="mb-8 p-6 bg-[#4B5563]/60 rounded-2xl border border-[#6B7280] animate-fade-in">
                        ...
                        <div className="flex justify-center gap-4 mb-10">
                          {words[currentWordIndex]?.letters.map((_, index) => (
                            <div
                              key={index}
                              className="w-16 h-16 border-2 border-[#6B7280] rounded-xl flex items-center justify-center text-2xl font-bold text-gray-200 bg-[#2D3748]/60 relative overflow-hidden"
                            >
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                          {letterGrid.map((letter, index) => (
                            <button
                              ...
                              className="w-full aspect-square bg-[#4B5563]/80 rounded-xl flex items-center justify-center text-xl font-bold text-gray-200 hover:bg-[#6B7280] transition-all hover:scale-105 border border-[#6B7280] shadow-lg"
                            >
                              {letter}
                            </button>
                          ))}
                          <button
                            ...
                            className="w-full aspect-square bg-[#2D3748]/80 rounded-xl flex items-center justify-center text-xl font-bold text-gray-200 hover:bg-[#4B5563] transition-all hover:scale-105 border border-[#4B5563] shadow-lg"
                          >
                            ←
                          </button>
                        </div>
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-xl text-gray-300">
                          You've spelled all the rhyming words correctly!
                        </p>
                      </div>
                    )}
                  </div>
      ...
      {showConfetti && (
        <div className="fixed inset-0 z-50 pointer-events-none">
          <Confetti
            ...
            numberOfPieces={200}
            gravity={0.3}
            colors={['#C7D2FE', '#DDD6FE', '#FECDD3', '#A5B4FC', '#E9D5FF']}
          />
        </div>
      )}
    </div>
...
export default PetRhymingWorksheet; 