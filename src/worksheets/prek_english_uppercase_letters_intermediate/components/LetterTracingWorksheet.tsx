...
const LetterTracingWorksheet: React.FC = () => {
  ...
  }, [currentPath.id, currentPathIndex, currentLetter.paths.length, handleLetterComplete, speak]);
  ...
  const getRelativePoint = (e: React.PointerEvent, svg: SVGSVGElement) => {
    ...
    return {
      x: point.x * scale.x,
      y: point.y * scale.y,
      threshold: Math.max(30, Math.min(50, rect.width / 8))
    };
  };
  ...
  }, [isDrawing, lastPoint, progress, getRelativePoint, getDistance, handlePathComplete]);
