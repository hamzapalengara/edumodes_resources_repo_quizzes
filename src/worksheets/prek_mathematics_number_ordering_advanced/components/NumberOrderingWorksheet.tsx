...
const LEVELS = [
  { numbers: [2, 1, 4, 3, 5], label: "Level 1" },
  { numbers: [5, 3, 1, 2, 4], label: "Level 2" },
  { numbers: [3, 5, 2, 1, 4], label: "Level 3" }
];
...
const NumberOrderingWorksheet: React.FC = () => {
  ...
  const [completedPositions, setCompletedPositions] = useState<boolean[]>([false, false, false, false, false]);
  ...
  const resetGame = useCallback(() => {
    ...
    setCompletedPositions([false, false, false, false, false]);
  ...
  return (
    ...
    >
      {({ score, markCorrect, markAttempted }) => (
        <div className="min-h-screen bg-gradient-to-b from-emerald-100 to-green-100">
          <WorksheetHeader />

          <div className="bg-emerald-100/80 p-4 shadow-md">
            ...
            <main className="px-0 md:px-4 py-8">
              <div className="max-w-2xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                  <h1 className="text-2xl md:text-3xl font-bold text-emerald-600 text-center mb-6">
                    Put the Numbers in Order! 🔢
                  </h1>

                  {/* Level Indicator */}
                  <div className="text-center mb-4">
                    <span className="text-sm md:text-base text-emerald-600 font-semibold bg-emerald-100 rounded-full px-4 py-1 inline-block">
                  ...
                  <div className="text-center mb-6">
                    <p className="text-sm md:text-base text-gray-600 bg-emerald-50 rounded-full px-4 py-2 inline-block">
                      Drag numbers to put them in order from 1 to 5
                    </p>
                  </div>
                  ...
                  <div className="flex justify-center gap-2 md:gap-4 mb-8 flex-wrap">
                    {LEVELS[currentLevel].numbers.map((num) => (
                      ...
                      >
                        <span className="font-bold text-2xl md:text-3xl text-emerald-600 pointer-events-none select-none">
                          {num}
                        </span>
                      </motion.div>
                    ))}
                  ...
                  <div className="flex justify-center gap-2 md:gap-4 flex-wrap">
                    {placedNumbers.map((num, index) => (
                      <motion.div
                        ...
                        className={`w-14 h-14 md:w-16 md:h-16 rounded-xl shadow-md flex items-center justify-center
                          ${completedPositions[index] ? 'bg-green-100 border-green-400' : 'bg-emerald-50 border-emerald-300'}
                      ...
                      >
                        {num !== null && (
                          <span className="font-bold text-2xl md:text-3xl text-emerald-600 select-none">
                            {num}
                          </span>
                        )}
                      </motion.div>
                  ...
                  {gameCompleted && (
                    <div className="mt-6 text-center">
                      <button
                        onClick={resetGame}
                        className="px-6 py-2 bg-emerald-500 text-white rounded-full hover:bg-emerald-600 active:bg-emerald-700 transition-colors"
                      >
                        Play Again 🔄
                      </button>
                    </div>
