      ...
      <header className="bg-white/30 backdrop-blur-md shadow-lg border border-white/50">
        <div className="py-4 px-4">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-xl font-bold text-indigo-900">Answer Key</h1>
          </div>
        </div>
      </header>
      ...
      <main className="px-0">
        <div className="bg-white/40 backdrop-blur-md p-2 md:p-4 mb-2 md:mb-4 shadow-lg border border-white/50">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-lg md:text-xl font-bold text-indigo-900 mb-2 md:mb-4">Number Tracing Guide</h3>
              ...
              {NUMBERS.map((number) => (
                <div key={number.value} className="bg-white/60 backdrop-blur-sm rounded-2xl p-2 md:p-4 border border-white/50 shadow-lg">
                  <div className="flex flex-col sm:flex-row items-center gap-2 md:gap-4 mb-2 md:mb-4">
                    <span className="text-4xl md:text-5xl font-bold text-indigo-900">{number.value}</span>
                    <span className="text-2xl md:text-3xl filter drop-shadow-md">{number.fruitEmoji}</span>
                    <span className="text-lg md:text-xl text-indigo-800 font-medium capitalize">{number.fruit}</span>
                  </div>
                  ...
                  <div className="space-y-1 md:space-y-2">
                    <h4 className="text-sm md:text-base text-indigo-900 font-semibold">Tracing Steps:</h4>
                    <ol className="list-decimal list-inside text-sm md:text-base text-indigo-800 space-y-0.5 md:space-y-1">
                      {number.paths.map((path, index) => (
                        <li key={path.id}>
