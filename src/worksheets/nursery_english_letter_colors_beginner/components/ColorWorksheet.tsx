...
const ColorWorksheet: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasPlayedIntro && words.length > 0) {
      speak("Welcome to color spelling! I'll help you learn to spell different colors. Click the speaker icon to hear each word. Then, click the letters to spell it. If you need help, click the hint button. Let's start with our first color.");
      setHasPlayedIntro(true);
      // Automatically speak the first word after 8 seconds
      setTimeout(() => {
        speak(`Your first color is ${words[0].word}. Look at the colored box for a visual hint.`);
      }, 8000);
    }
      ...
      if (isWordCorrect) {
        ...
        setTimeout(() => {
          ...
          if (currentWordIndex < words.length - 1) {
            ...
            setTimeout(() => {
              speak(`Next color is ${nextWord.word}. Look at the colored box.`);
            }, 500);
          } else {
            setIsComplete(true);
            setShowCompletionOverlay(true);
            speak("Congratulations! You've learned all the colors!");
                        ...
                        <div className="flex items-center justify-between mb-8">
                          <div className="flex items-center gap-6">
                            ...
                            <div className="space-y-4">
                              <span className="text-lg font-medium text-gray-700">Click to hear the color</span>
                              <div 
                                className="w-32 h-12 rounded-xl shadow-inner"
                                style={{ backgroundColor: words[currentWordIndex]?.color }}
                              />
                            </div>
                          </div>
                        ...
                        <div className={`absolute inset-0 transition-all duration-1000 ${
                          ...
                          <div className="text-center py-12 space-y-6">
                            ...
                            <div className="flex justify-center gap-4">
                              ...
                              <div className="flex items-center gap-2 bg-blue-100 px-6 py-3 rounded-xl">
                                <span className="text-2xl">🎯</span>
                                <span className="text-blue-700 font-medium">All Colors Mastered</span>
                              </div>
                            </div>
                          </div>
                        </div>
      ...
      {showConfetti && (
        <div className="fixed inset-0 z-50 pointer-events-none">
          <Confetti
            ...
            numberOfPieces={200}
            gravity={0.3}
            colors={['#3B82F6', '#EC4899', '#6B7280', '#F59E0B', '#84CC16', '#F43F5E', '#14B8A6', '#D4B89C', '#06B6D4', '#1E3A8A']}
          />
        </div>
      )}
    </div>
