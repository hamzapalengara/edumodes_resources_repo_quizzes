...
// Fruit theme colors
const WORD_COLORS = [
  { bg: 'bg-orange-200', text: 'text-orange-800' },  // Orange
  { bg: 'bg-pink-200', text: 'text-pink-800' },     // Strawberry
  { bg: 'bg-yellow-200', text: 'text-yellow-800' }, // Banana
  { bg: 'bg-purple-200', text: 'text-purple-800' }, // Fig
  { bg: 'bg-green-200', text: 'text-green-800' },   // Avocado
];

// Grid size for 10 words
const GRID_SIZE = 8;

// New fruit-themed word list (different from the first set)
const WORD_LIST = [
  { word: 'FIG', emoji: '🫐' },
  { word: 'DATES', emoji: '🌴' },
  { word: 'GUAVA', emoji: '🫐' },
  ...
  { word: 'CHERRY', emoji: '🍒' },
  { word: 'BERRY', emoji: '🫐' },
  { word: 'DRAGON', emoji: '🐉' },  // Dragon fruit
  { word: 'STAR', emoji: '⭐' },    // Star fruit
];
...
const WordSearchWorksheet: React.FC = () => {
  ...
  const provideFeedback = (isCorrect: boolean, word?: string) => {
    if (isCorrect && word) {
      const phrases = [
        `Yummy! You found ${word}! That's a delicious fruit!`,
        `Sweet! You found ${word}! Keep searching!`,
        `Fantastic! You found ${word}! What a juicy find!`,
        `Amazing! You found ${word}! You're doing great!`,
        `Wonderful! You found ${word}! Keep picking those fruits!`
      ];
  ...
  const handleCellMouseUp = (
    ...
    if (wordPosition) {
      ...
      // Update word position status with color index and add emojis to cells
      ...
      // Update grid with emojis for found word
      const newGrid = [...grid];
      wordPosition.cells.forEach(({ row, col }) => {
        newGrid[row][col].emoji = wordPosition.emoji;
      });
      setGrid(newGrid);
      ...
      if (foundWords.length + 1 === WORD_LIST.length) {
        ...
        setTimeout(() => {
          speak("Amazing! You've found all the delicious fruits! What a healthy harvest!", 1, 1.2, 1);
        }, 1000);
      }
    } else {
      markIncorrect();
          ...
          <div className="px-0 md:px-4 max-w-4xl mx-auto">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 mb-4">
              ...
              <div className="text-center mb-3">
                <p className="text-white text-lg font-medium">
                  Find these exotic fruits in the grid below! 🍊
                </p>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`
                          ...
                          text-lg md:text-xl font-bold select-none touch-none relative
                          ${cell.isSelected ? 'bg-orange-300 text-orange-800' : 
                            wordPosition ? `${WORD_COLORS[colorIndex].bg} ${WORD_COLORS[colorIndex].text}` :
                            'bg-white text-gray-700'}
                          ${cell.isPartOfWord ? 'hover:bg-orange-100' : ''}
                        `}
            ...
            {isComplete && (
              <motion.div
                ...
                className="mt-4 p-4 bg-orange-100 text-orange-800 rounded-xl text-center"
              >
                <h2 className="text-2xl font-bold mb-2">🍊 Fantastic! 🍊</h2>
                <p>You've found all the exotic fruits!</p>
              </motion.div>
            )}
          </div>
        )}
