...
const CountingWorksheet: React.FC = () => {
  ...
  const handleItemClick = (itemIndex: number, markAttempted: () => void, markCorrect: () => void) => {
    const newItems = [...items];
    const item = newItems[itemIndex];

    if (!item.isCompleted && !isSpeaking && item.isActive) {
      markAttempted(); // Mark an attempt when the user clicks
      ...
      if (item.currentCount === item.count) {
        ...
        markCorrect(); // Mark correct when the count matches
  ...
  return (
    <WorksheetTracker
      totalQuestions={10} // One question per counting item
      pointsPerQuestion={10}
      onSummaryGenerated={(summary: WorksheetSummary) => {
        console.log('Counting Objects Worksheet Summary:', summary);
      }}
    >
              ...
              {items.map((item, index) => (
                <TouchContainer key={item.id}>
                  <div 
                    ...
                    onClick={() => handleItemClick(index, markAttempted, markCorrect)}
                  >
