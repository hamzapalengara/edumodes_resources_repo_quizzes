...
const dialogues: Dialogue[] = [
  {
    ...
    emoji: '👨‍⚕️',
    expectedResponse: "I am not feeling well today"
  },
  {
    id: 2,
    speaker: 'doctor',
    text: "Can you tell me where it hurts?",
    emoji: '🤒',
    expectedResponse: "My stomach hurts a lot"
  },
  {
    id: 3,
    speaker: 'doctor',
    text: "I see. Did you eat anything different yesterday?",
    emoji: '🍽️',
    expectedResponse: "Yes I ate some spicy food"
  },
  {
    id: 4,
    speaker: 'doctor',
    text: "Let me check your temperature. Can you open your mouth?",
    emoji: '🌡️',
    expectedResponse: "Yes I will open my mouth"
  },
  {
    id: 5,
    speaker: 'doctor',
    text: "You have a slight fever. Have you been drinking enough water?",
    emoji: '💧',
    expectedResponse: "No I need to drink more water"
  },
  {
    id: 6,
    speaker: 'doctor',
    text: "I'll give you some medicine. Will you take it on time?",
    emoji: '💊',
    expectedResponse: "Yes I will take the medicine"
  }
];

const SpeakingDoctorWorksheet: React.FC = () => {
  ...
  // Play doctor's voice on initial load
  useEffect(() => {
    if (activeDialogueIndex === 0) {
      speakDoctorText(dialogues[activeDialogueIndex].text);
    }
  }, []);
  ...
  const speakDoctorText = (text: string) => {
    ...
    const doctorVoice = voices.find(voice => 
  ...
  const speakPatientResponse = (text: string) => {
    ...
    // Higher pitch for patient's voice
    utterance.pitch = 1.2;
    utterance.rate = 1.0;
              ...
              if (!completedDialogues[activeDialogueIndex]) {
                ...
                setTimeout(() => {
                  ...
                  if (nextIndex < dialogues.length) {
                    ...
                    setSpokenText('');
                    speakDoctorText(dialogues[nextIndex].text);
                  }
                }, 1500);
              }
            }
          ...
          return (
            <div className="px-0 py-2 md:py-4">
              <div className="max-w-4xl mx-auto">
                ...
                <div className="mx-2 md:mx-4 bg-cyan-50 p-3 md:p-4 shadow-md mb-4 rounded-lg">
                  <ScoreDisplay 
                    score={score}
                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                  <h1 className="text-xl md:text-2xl font-bold text-center text-cyan-600 p-4 border-b border-cyan-100">
                    ...
                    {dialogues.map((dialogue, index) => (
                      <motion.div 
                        ...
                        className={`p-3 md:p-4 ${
                          completedDialogues[index] ? 'bg-cyan-50' : 'bg-white'
                        } ${index > activeDialogueIndex ? 'hidden' : ''}`}
                      >
                        {/* Doctor's Line */}
                        <div className="flex items-start gap-3 mb-4">
                          <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-cyan-100 flex items-center justify-center shrink-0">
                            👨‍⚕️
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="bg-cyan-50 rounded-lg p-3">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <p className="text-base md:text-lg font-semibold text-cyan-800 break-words">
                                  {dialogue.text}
                                </p>
                              </div>
                              <motion.button
                                onClick={() => speakDoctorText(dialogue.text)}
                                className="mt-2 bg-cyan-200 text-cyan-700 px-3 py-1.5 md:px-4 md:py-2 rounded-full flex items-center gap-2 text-sm md:text-base"
                            ...
                            <div className="bg-blue-50 rounded-lg p-3">
                              ...
                              {isEditing && activeDialogueIndex === index ? (
                                <div className="mb-3">
                                  <input
                                    ...
                                    className="w-full p-2 border border-cyan-200 rounded-lg text-base"
                                  />
                                  <div className="flex gap-2 mt-2">
                                    <motion.button
                                      onClick={handleEditSubmit}
                                      className="bg-cyan-500 text-white px-3 py-1.5 rounded-full text-sm"
                                      whileHover={{ scale: 1.05 }}
                                      whileTap={{ scale: 0.95 }}
                                    >
                                      ✅ Submit
                                ...
                                <p className="text-base md:text-lg font-semibold text-cyan-800 min-h-[2rem] break-words">
                              ...
                              <div className="flex flex-wrap gap-2 mt-3">
                                    ...
                                    {spokenText && (
                                      <>
                                        <motion.button
                                          ...
                                          className="bg-cyan-500 text-white px-4 py-1.5 md:py-2 rounded-full flex items-center gap-2 text-sm md:text-base"
                                          whileHover={{ scale: 1.05 }}
                                          whileTap={{ scale: 0.95 }}
                                        >
                                          ✅ Submit
                                ...
                                {completedDialogues[index] && (
                                  <motion.button
                                    onClick={() => speakPatientResponse(lastSpokenResponse)}
                                    className="bg-cyan-200 text-cyan-700 px-4 py-1.5 md:py-2 rounded-full flex items-center gap-2 text-sm md:text-base"
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                  >
                                    🔊 Play My Response
...
export default SpeakingDoctorWorksheet; 