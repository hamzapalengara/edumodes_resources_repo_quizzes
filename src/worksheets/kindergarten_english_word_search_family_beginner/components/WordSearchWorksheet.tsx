...
// Theme colors for family members
const WORD_COLORS = [
...
const WordSearchWorksheet: React.FC = () => {
  ...
  const provideFeedback = (isCorrect: boolean) => {
    if (isCorrect) {
      const messages = [
        "Great job! You found a family member!",
        "Excellent! Keep searching!",
        "You're doing great with your family tree!",
        "Wonderful finding!"
      ];
      speak(messages[Math.floor(Math.random() * messages.length)], 1.2, 1, 1);
    } else {
      speak("Try again!", 0.8, 1, 0.8);
    }
  };
  ...
  const handleCellMouseUp = (
    ...
    if (wordPosition) {
      ...
      if (foundWords.length + 1 === WORD_LIST.length) {
        ...
        setTimeout(() => {
          speak("Congratulations! You've found all the family members!", 1, 1.2, 1);
        }, 1000);
      }
    } else {
      markIncorrect();
          ...
          <div className="px-0 md:px-4 max-w-4xl mx-auto">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 mb-4">
              ...
              <div className="text-center mb-3">
                <p className="text-white text-lg font-medium">
                  Find these family members in the grid below! 👨‍👩‍👧‍👦
                </p>
            ...
            {isComplete && (
              <motion.div
                ...
                className="mt-4 p-4 bg-rose-100 text-rose-800 rounded-xl text-center"
              >
                <h2 className="text-2xl font-bold mb-2">🎉 Congratulations! 🎉</h2>
                <p>You've found all the family members!</p>
              </motion.div>
            )}
          </div>
        )}
