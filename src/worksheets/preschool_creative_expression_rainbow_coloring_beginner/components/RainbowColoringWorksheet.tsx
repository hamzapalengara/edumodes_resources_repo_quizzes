...
const RainbowColoringWorksheet: React.FC = () => {
  ...
  const handlePatternClick = useCallback((index: number) => {
    ...
    if (color.name === expectedColor) {
      // Update the sequence
      const newSequence = [...currentPattern.sequence];
      ...
      if (!newSequence.includes('')) {
        setTimeout(() => {
          if (patternIndex < patterns.length - 1) {
            setPatternIndex(prev => prev + 1);
            setCurrentPattern(patterns[patternIndex + 1]);
            const utterance = new SpeechSynthesisUtterance('Wonderful! Try the next pattern!');
            window.speechSynthesis.speak(utterance);
          } else {
            setPatternComplete(true);
            const utterance = new SpeechSynthesisUtterance('Amazing! You completed all the patterns!');
            window.speechSynthesis.speak(utterance);
          }
        }, 1000);
      }
      ...
      const utterance = new SpeechSynthesisUtterance(`Try again! We need ${expectedColor} here.`);
      window.speechSynthesis.speak(utterance);
    }
  }, [selectedColor, currentPattern, patterns, patternIndex, triggerConfetti]);
