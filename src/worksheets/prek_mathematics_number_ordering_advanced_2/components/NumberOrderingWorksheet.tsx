...
const LEVELS = [
  { numbers: [7, 5, 9, 6, 8, 10], label: "Level 1" },
  { numbers: [10, 8, 5, 7, 6, 9], label: "Level 2" },
  { numbers: [6, 9, 7, 10, 5, 8], label: "Level 3" }
];

const NumberOrderingWorksheet: React.FC = () => {
  const [currentLevel, setCurrentLevel] = useState(0);
  const [placedNumbers, setPlacedNumbers] = useState<(number | null)[]>([null, null, null, null, null, null]);
  const [selectedNumber, setSelectedNumber] = useState<number | null>(null);
  const [completedPositions, setCompletedPositions] = useState<boolean[]>([false, false, false, false, false, false]);
  ...
  const resetGame = useCallback(() => {
    setPlacedNumbers([null, null, null, null, null, null]);
    setSelectedNumber(null);
    setCompletedPositions([false, false, false, false, false, false]);
  ...
  const handleDragEnd = useCallback((
    ...
    targetRefs.current.forEach((ref, index) => {
      if (ref && !placedNumbers[index] && isCloseToTarget(x, y, ref)) {
        ...
        // Check if correct
        const isCorrect = number === index + 5; // Offset by 5 since we start at 5
    ...
    >
      {({ score, markCorrect, markAttempted }) => (
        <div className="min-h-screen bg-gradient-to-b from-amber-100 to-orange-100">
          ...
          <div className="bg-amber-100/80 p-4 shadow-md">
            ...
            <main className="px-0 md:px-4 py-8">
              <div className="max-w-2xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                  <h1 className="text-2xl md:text-3xl font-bold text-amber-600 text-center mb-6">
                    Put the Numbers in Order! 🔢
                  </h1>

                  {/* Level Indicator */}
                  <div className="text-center mb-4">
                    <span className="text-sm md:text-base text-amber-600 font-semibold bg-amber-100 rounded-full px-4 py-1 inline-block">
                  ...
                  <div className="text-center mb-6">
                    <p className="text-sm md:text-base text-gray-600 bg-amber-50 rounded-full px-4 py-2 inline-block">
                      Drag numbers to put them in order from 5 to 10
                    </p>
                  </div>
                  ...
                  <div className="flex justify-center gap-2 md:gap-4 mb-8 flex-wrap">
                    {LEVELS[currentLevel].numbers.map((num) => (
                      <motion.div
                        ...
                        className={`w-14 h-14 md:w-16 md:h-16 rounded-xl shadow-md flex items-center justify-center
                      ...
                      >
                        <span className="font-bold text-2xl md:text-3xl text-amber-600 pointer-events-none select-none">
                          {num}
                        </span>
                      </motion.div>
                    ))}
                  ...
                  <div className="flex justify-center gap-2 md:gap-4 flex-wrap">
                    {placedNumbers.map((num, index) => (
                      <motion.div
                        ...
                        className={`w-14 h-14 md:w-16 md:h-16 rounded-xl shadow-md flex items-center justify-center
                          ${completedPositions[index] ? 'bg-orange-100 border-orange-400' : 'bg-amber-50 border-amber-300'}
                      ...
                      >
                        {num !== null && (
                          <span className="font-bold text-2xl md:text-3xl text-amber-600 select-none">
                            {num}
                          </span>
                        )}
                      </motion.div>
                  ...
                  {gameCompleted && (
                    <div className="mt-6 text-center">
                      <button
                        onClick={resetGame}
                        className="px-6 py-2 bg-amber-500 text-white rounded-full hover:bg-amber-600 active:bg-amber-700 transition-colors"
                      >
                        Play Again 🔄
                      </button>
                    </div>
