...
const SUCCESS_MESSAGES = [
  "Great job finding that even number! 🌟",
  "You're getting better at this! ⭐",
  "Keep going, you're doing amazing! 🎯",
  "That's correct! You found an even number! 🎨",
...
const EvenNumbersWorksheet: React.FC = () => {
  ...
  const [message, setMessage] = useState<string>('Find all the even numbers (2, 4, 6, 8, 10)!');
  ...
  const handleTileClick = (
    ...
    if (tile.isEven) {
      ...
      markIncorrect();
      playSound(false);
      const message = "That's not an even number. Try again!";
      setMessage(message);
      speak(message);
    }

    // Check if all even numbers are found
    const allEvenNumbersFound = newGrid
      .filter(t => t.isEven)
      .every(t => t.isSelected);

    if (allEvenNumbersFound) {
      const message = "Congratulations! You've found all the even numbers! 🎉";
      setMessage(message);
      speak(message);
    }
  };
        ...
        >
          {({ markCorrect, markIncorrect, markAttempted, score }) => (
            <div className="space-y-4">
              ...
              {/* Message Display */}
              <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 text-center text-purple-800 font-bold shadow-lg">
                {message}
              </div>
              ...
              {/* Instructions */}
              <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 text-purple-700 shadow-lg">
                <h2 className="font-bold mb-2">How to Play:</h2>
                <ul className="list-disc list-inside space-y-1">
                  <li>Click on the even numbers (2, 4, 6, 8, 10)</li>
                  <li>Even numbers can be divided evenly by 2</li>
                  <li>Odd numbers (1, 3, 5, 7, 9) are not even numbers</li>
                </ul>
              </div>
            </div>
          )}
