...
export const NUMBER_FIVE = {
  ...
  viewBox: '0 0 200 200',
  paths: [
    { id: 'top_line', d: 'M140 40L60 40', order: 1 },
    { id: 'vertical', d: 'M60 40L60 100', order: 2 },
    { id: 'curve', d: 'M60 100C60 100 80 100 100 100C120 100 130 120 130 140C130 160 120 170 100 170L60 170', order: 3 }
  ]
};
      ...
      {({ markCorrect, markAttempted, score }) => {
        ...
        const handlePathComplete = useCallback(() => {
          ...
          if (currentPath && !filledPaths.includes(currentPath.id)) {
            ...
            if (currentPathIndex < NUMBER_FIVE.paths.length - 1) {
              setCurrentPathIndex(prev => prev + 1);
              setLastPoint(0);
              setProgress(0);
              setLastValidPoint(null);
              speak("Number 5. Let's trace the next part.");
            } else {
              setShowSuccess(true);
              ...
              if (newPracticeCount < REQUIRED_PRACTICES) {
                speak(`Number 5! You've completed it ${newPracticeCount} time${newPracticeCount > 1 ? 's' : ''}. ${REQUIRED_PRACTICES - newPracticeCount} more to go!`);
              } else {
                speak("Number 5! You've completed all five practices! Now let's find all the number fives in the grid below!");
                setIsCompleted(true);
              }
        ...
        const handleNumberClick = useCallback((index: number) => {
          if (foundFives.includes(index)) return;
          
          if (NUMBER_GRID[index] === '5 🍎') {
            setFoundFives(prev => [...prev, index]);
            markCorrect();
            speak("Correct! You found a number 5!");
            
            if (foundFives.length + 1 === 5) {
              speak("Amazing! You've found all the number fives!");
            }
          } else {
            speak("That's not a 5, try again!");
          }
        }, [foundFives, markCorrect, speak]);
        ...
        useEffect(() => {
          if (pathRef.current && NUMBER_FIVE.paths[currentPathIndex]?.id) {
            const length = pathRef.current.getTotalLength();
            setPathLengths(prev => ({
              ...prev,
              [NUMBER_FIVE.paths[currentPathIndex].id]: length
            }));
          }
        }, [currentPathIndex]);
        ...
        // Initial instruction
        useEffect(() => {
          speak("Let's trace the number 5");
        }, [speak]);
        ...
        const playAnimation = useCallback(async () => {
          ...
          for (let i = 0; i < NUMBER_FIVE.paths.length; i++) {
              ...
              <div className="bg-white/40 backdrop-blur-md p-2 md:p-4 mb-2 md:mb-4 shadow-lg border border-white/50">
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 md:gap-6 mb-2 md:mb-4">
                    <div className="text-4xl sm:text-5xl font-bold text-indigo-900">{NUMBER_FIVE.value}</div>
                    <div className="flex flex-wrap justify-center gap-1 items-center max-w-[200px] sm:max-w-none">
                      {[...Array(NUMBER_FIVE.fruitCount)].map((_, index) => (
                        <div key={index} className="text-2xl sm:text-3xl filter drop-shadow-md">{NUMBER_FIVE.fruitEmoji}</div>
                      ))}
                    </div>
                    <div className="text-lg sm:text-xl font-medium text-indigo-800 capitalize">{NUMBER_FIVE.fruit}</div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    ...
                    <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-white/50 shadow-lg">
                      <div className="text-center mb-4">
                        ...
                        <p className="text-sm text-indigo-700">See how to write number 5</p>
                      </div>
                    ...
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-gray-200 shadow-lg">
                      ...
                      <div className="mt-2 text-center">
                        <p className="text-sm font-medium text-indigo-700">
                          {currentPathIndex === 0 && "Start with the top line ➡️"}
                          {currentPathIndex === 1 && "Draw the vertical line ⬇️"}
                          {currentPathIndex === 2 && "Complete the curve ↘️"}
                          {currentPathIndex >= NUMBER_FIVE.paths.length && "Great job! Try again! 🎉"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 bg-white/60 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-white/50 shadow-lg relative">
                    ...
                    <div className="text-center mb-4">
                      <h3 className="text-lg md:text-xl font-bold text-indigo-900">Find the Number 5s!</h3>
                      <p className="text-sm text-indigo-700 mb-2">Click on all the number 5s you can find</p>
                      <div className="inline-flex items-center gap-2 bg-indigo-100 px-3 py-1 rounded-full">
                        ...
                        <span className="text-sm font-bold text-indigo-900">{foundFives.length} / 5</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-5 gap-2 max-w-[350px] mx-auto">
                      {NUMBER_GRID.map((number, index) => (
                        <button
                          ...
                          disabled={!isCompleted || foundFives.includes(index)}
                          className={`
                            aspect-square w-full text-xl font-bold rounded-lg
                            flex items-center justify-center transition-all
                            ${foundFives.includes(index)
                    ...
                    {foundFives.length === 5 && (
                      <div className="mt-4 text-center">
                        <p className="text-green-600 font-semibold">
                          🎉 Fantastic! You've found all the number 5s! 🎉
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    {showSuccess && (
                      <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
                        {confetti.map((item) => (
                          ...
                          >
                            {item.type === 'emoji' ? NUMBER_FIVE.fruitEmoji : NUMBER_FIVE.value}
                          </motion.div>
                        ))}
                      </div>
                    )}
...
export default NumberFiveWorksheet; 