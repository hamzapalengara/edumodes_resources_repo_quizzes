...
const NumberFiveAnswerKey: React.FC = () => {
  return (
      ...
      <main className="px-0">
        <div className="bg-white/40 backdrop-blur-md p-2 md:p-4 mb-2 md:mb-4 shadow-lg border border-white/50">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-lg md:text-xl font-bold text-indigo-900 mb-2 md:mb-4">Number 5 Tracing Guide</h3>
            
            <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-white/50 shadow-lg">
              {/* Number Display */}
              <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
                <span className="text-5xl md:text-6xl font-bold text-indigo-900">{NUMBER_FIVE.value}</span>
                  ...
                  <div key={path.id} className="flex items-start gap-4">
                    ...
                    <div className="flex-grow">
                      <h4 className="text-base md:text-lg font-semibold text-indigo-900 mb-2">
                        {index === 0 && "Draw the Top Line"}
                        {index === 1 && "Add the Vertical Line"}
                        {index === 2 && "Complete with the Curve"}
                      </h4>
                      <div className="relative aspect-square max-w-[350px]">
                        <svg viewBox={NUMBER_FIVE.viewBox} className="w-full h-full">
                          {/* Background decoration */}
                          <circle cx="100" cy="100" r="80" fill="rgba(99, 102, 241, 0.1)" />
                          
                          {/* Previous paths (completed) */}
                          {NUMBER_FIVE.paths.slice(0, index).map(prevPath => (
                            <path
                              key={prevPath.id}
                              d={prevPath.d}
                              fill="none"
                          ...
                          {/* Future paths (faded) */}
                          {NUMBER_FIVE.paths.slice(index + 1).map(nextPath => (
                            <path
                              key={nextPath.id}
                              d={nextPath.d}
                              fill="none"
                      ...
                      <p className="text-sm md:text-base text-indigo-800 mt-2">
                        {index === 0 && "Start from the right and draw straight across to the left."}
                        {index === 1 && "Draw a straight line down from the left end."}
                        {index === 2 && "Draw a curve from the bottom of the vertical line to the right and back to the left."}
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-6 bg-white/60 backdrop-blur-sm rounded-2xl p-4 md:p-6 border border-white/50 shadow-lg">
              ...
              <ul className="space-y-2 text-sm md:text-base text-indigo-800">
                ...
                <li className="flex items-center gap-2">
                  <span className="text-xl md:text-2xl">🍎</span>
                  <span>Remember: The number 5 represents five apples!</span>
                </li>
              </ul>
            </div>
          </div>
...
export default NumberFiveAnswerKey; 