...
const WORD_COLORS = [
  ...
  { bg: 'bg-amber-200', text: 'text-amber-800' }, // For autumn leaves
];

const GRID_SIZE = 7;

// Forest-themed word list
const WORD_LIST = [
  { word: 'TREE', emoji: '🌳' },
  { word: 'LEAF', emoji: '🍃' },
  { word: 'BIRD', emoji: '🐦' },
...
const WordSearchWorksheet: React.FC = () => {
  ...
  const provideFeedback = (isCorrect: boolean) => {
    if (isCorrect) {
      speak("Great job! You found a word!", 1.2, 1, 1);
    } else {
  ...
  const handleCellMouseUp = (
    ...
    if (wordPosition) {
      ...
      if (foundWords.length + 1 === WORD_LIST.length) {
        ...
        setTimeout(() => {
          speak("Congratulations! You've found all the forest words!", 1, 1.2, 1);
        }, 1000);
      }
    } else {
      markIncorrect();
          ...
          <div className="px-0 md:px-4 max-w-4xl mx-auto">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 mb-4">
              ...
              <div className="text-center mb-3">
                <p className="text-white text-lg font-medium">
                  Find these forest words in the grid below! 🌲
                </p>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`
                          ...
                          ${cell.isSelected ? 'bg-emerald-300 text-emerald-800' : 
                            wordPosition ? `${WORD_COLORS[colorIndex].bg} ${WORD_COLORS[colorIndex].text}` :
                            'bg-white text-gray-700'}
                          ${cell.isPartOfWord ? 'hover:bg-emerald-100' : ''}
                        `}
            ...
            {isComplete && (
              <motion.div
                ...
                className="mt-4 p-4 bg-emerald-100 text-emerald-800 rounded-xl text-center"
              >
                <h2 className="text-2xl font-bold mb-2">🎉 Congratulations! 🎉</h2>
                <p>You've found all the forest words!</p>
              </motion.div>
            )}
          </div>
        )}
