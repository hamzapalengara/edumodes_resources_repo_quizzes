...
// Increased grid size for more words
const GRID_SIZE = 8;

// Sports-themed word list (10 words)
const WORD_LIST = [
  { word: 'BALL', emoji: '⚽' },
  { word: 'SWIM', emoji: '🏊' },
  { word: 'JUMP', emoji: '🦘' },
...
const WordSearchWorksheet: React.FC = () => {
  ...
  const provideFeedback = (isCorrect: boolean, word?: string) => {
    if (isCorrect && word) {
      const phrases = [
        `Great job! You found the word ${word}! You're a sports star!`,
        `Score! You found ${word}! Keep going!`,
        `Touchdown! You found ${word}! That's awesome!`,
        `Goal! You found ${word}! You're doing great!`,
        `Home run! You found ${word}! Nice work!`
      ];
  ...
  const handleCellMouseUp = (
    ...
    if (wordPosition) {
      ...
      if (foundWords.length + 1 === WORD_LIST.length) {
        ...
        setTimeout(() => {
          speak("Champion! You've found all the sports words!", 1, 1.2, 1);
        }, 1000);
      }
    } else {
      markIncorrect();
          ...
          <div className="px-0 md:px-4 max-w-4xl mx-auto">
            ...
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-4 mb-4">
              ...
              <div className="text-center mb-3">
                <p className="text-white text-lg font-medium">
                  Find these sports words in the grid below! 🏆
                </p>
            ...
            {isComplete && (
              <motion.div
                ...
                className="mt-4 p-4 bg-blue-100 text-blue-800 rounded-xl text-center"
              >
                <h2 className="text-2xl font-bold mb-2">🏆 Champion! 🏆</h2>
                <p>You've found all the sports words!</p>
              </motion.div>
            )}
          </div>
        )}
