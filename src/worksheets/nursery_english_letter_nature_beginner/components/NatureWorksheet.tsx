...
const WORD_LIST = [
  { word: 'TREE', pattern: '-ree', hint: 'Grows tall in nature' },
  { word: 'FREE', pattern: '-ree', hint: 'Birds flying in the sky' },
  { word: 'BEE', pattern: '-ree', hint: 'Buzzing near flowers' },
  { word: 'RAIN', pattern: '-ain', hint: 'Water from clouds' },
  { word: 'PAIN', pattern: '-ain', hint: 'When you fall outside' },
  { word: 'GAIN', pattern: '-ain', hint: 'Plants getting water' },
  { word: 'WIND', pattern: '-ind', hint: 'Moves leaves and kites' },
  { word: 'FIND', pattern: '-ind', hint: 'Discover nature treasures' },
  { word: 'KIND', pattern: '-ind', hint: 'Nice to animals' }
];
...
const NatureWorksheet: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasPlayedIntro && words.length > 0) {
      speak("Welcome to nature spelling! I'll help you learn words about the outdoors. Click the speaker icon to hear each word. Then, click the letters to spell it. If you need help, click the hint button. Let's start with our first word.");
      setHasPlayedIntro(true);
      // Automatically speak the first word after 8 seconds (giving more time to process instructions)
      setTimeout(() => {
        speak(`Your first word is ${words[0].word}. It has the pattern ${words[0].pattern}`);
      }, 8000);
    }
...
export default NatureWorksheet; 