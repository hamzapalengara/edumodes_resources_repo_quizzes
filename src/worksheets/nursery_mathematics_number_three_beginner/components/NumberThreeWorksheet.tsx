...
import './NumberThreeWorksheet.css';
...
// Define the number three path with correct writing style
const NUMBER_THREE = {
  value: '3',
  viewBox: '0 0 200 200',
  paths: [
...
// Update grid items for number three
const GRID_ITEMS = [
  { id: 1, value: '3', isTarget: true },
  { id: 2, value: '3', isTarget: true },
  { id: 3, value: '3', isTarget: true },
  { id: 4, value: '3', isTarget: true },
  { id: 5, value: '3', isTarget: true },
...
const NumberThreeWorksheet: React.FC = () => {
  const svgRef = useRef<SVGSVGElement>(null);
  const pathRef = useRef<SVGPathElement>(null);
  const [isDrawing, setIsDrawing] = useState(false);
  const [attempts, setAttempts] = useState(0);
  const [currentPath, setCurrentPath] = useState<NumberPath | null>(NUMBER_THREE.paths[0]);
  ...
  useEffect(() => {
    ...
    NUMBER_THREE.paths.forEach(path => {
  ...
  const handlePathComplete = useCallback(() => {
    ...
    if (currentPath.id === 'bottom_half') {
      setIsDrawing(false);
      setIsPathCompleting(false);

      // Create mini celebration for each successful attempt
      ...
      // Mark as completed and update attempts
      ...
      // Speak completion message and reset after voice completion
      const utterance = new SpeechSynthesisUtterance("Excellent! You wrote number three!");
      utterance.rate = 0.9;
      utterance.pitch = 1;

      // Add event listener for voice completion
      utterance.onend = () => {
        // Cleanup celebration
        document.body.removeChild(container);
        
        if (attempts < TOTAL_ATTEMPTS - 1) {
          // Reset for next attempt after voice completion
          setFilledPaths([]);
          setCurrentPath(NUMBER_THREE.paths[0]);
          setProgress(0);
          setLastPoint(0);
          
          // Start next attempt message
          const nextAttemptUtterance = new SpeechSynthesisUtterance("Let's practice again! Start from the dot.");
          nextAttemptUtterance.rate = 0.9;
          nextAttemptUtterance.pitch = 1;
          window.speechSynthesis.speak(nextAttemptUtterance);
        } else {
          // Final celebration and transition to identification game
          const transitionUtterance = new SpeechSynthesisUtterance("Amazing! You've completed all practice attempts. Now let's find all the threes!");
          transitionUtterance.rate = 0.9;
          transitionUtterance.pitch = 1;
          transitionUtterance.onend = () => {
            ...
            if (!confettiTimeoutRef.current) {
              confettiTimeoutRef.current = setTimeout(() => {
                ...
                finalContainer.style.zIndex = '50';
      ...
      // Start the completion voice message
      window.speechSynthesis.speak(utterance);
    } else {
      // Move to next path
      const nextPathIndex = NUMBER_THREE.paths.findIndex(p => p.id === currentPath.id) + 1;
      if (nextPathIndex < NUMBER_THREE.paths.length) {
        setCurrentPath(NUMBER_THREE.paths[nextPathIndex]);
        setProgress(0);
        setLastPoint(0);
        setIsPathCompleting(false);
        setIsDrawing(false);
        speak("Good! Now let's complete the bottom half!");
      }
    }
  }, [currentPath, speak, attempts]);

  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    if (!svgRef.current || !pathRef.current || !currentPath || isPathCompleting) return;
    ...
    // Check distance to current path's start point
    const startPoint = currentPath.startPoint;
    const distanceToStart = getDistance(point, startPoint);
    
    if (distanceToStart < TOLERANCE * 1.5) {
      setIsDrawing(true);
      setProgress(0);
      setLastPoint(0);
  ...
  }, [currentPath, getRelativePoint, getDistance, progress, lastPoint, isPathCompleting]);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent) => {
    if (isDrawing && svgRef.current) {
      ...
      if (!isPathCompleting && !filledPaths.includes('top_half')) {
        setIsDrawing(false);
      }
    }
  ...
  const handleItemClick = useCallback((id: number, isTarget: boolean) => {
    ...
    if (selectedItems.includes(id)) {
      ...
      if (isTarget) {
        ...
        if (correctCount === 1) {
          speak("Yes! That's a three! Find more!");
        } else if (correctCount === 3) {
          speak("Great job! Just a couple more threes to find!");
        } else if (correctCount === 4) {
          speak("Almost there! Find one more three!");
        } else {
          speak("Yes! That's a three!");
        }
        trackerFunctionsRef.current.markCorrect();
      } else {
        speak("Oops! That's not a three. Keep looking!");
      ...
      if (correctSelections.length === 5) {
        ...
        speak("Amazing! You found all the threes!");
        
        // Create falling threes celebration with more threes and longer duration
        const colors = ['#2563eb', '#7c3aed', '#ec4899', '#10b981', '#f97316', '#ef4444'];
        const container = document.createElement('div');
        container.style.position = 'fixed';
        container.style.top = '0';
            ...
            <div className="px-0 md:px-4 py-4 max-w-4xl mx-auto">
              <div className="bg-white rounded-lg shadow-md p-4">
                <h1 className="text-2xl md:text-3xl font-bold text-center text-blue-600 mb-6">
                  Learning to Write Number Three
                </h1>
                      ...
                      <div className="bg-white/70 backdrop-blur-sm rounded-xl p-4 shadow-inner">
                        ...
                        <div className="relative w-full aspect-square max-w-md mx-auto bg-white rounded-xl overflow-hidden border-4 border-dashed border-indigo-200 shadow-lg">
                          ...
                          >
                            ...
                            {filledPaths.includes('top_half') && !filledPaths.includes('bottom_half') && (
                              <g>
                                {/* Outer pulsing rings */}
                                <circle
                                  cx="60"
                ...
                <div className={`transition-all duration-500 ${gamePhase === 'identification' ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 mb-6">
                    <h2 className="text-lg font-semibold text-indigo-600 mb-4 flex items-center gap-2">
                      <span className="text-2xl">🔍</span>
                      Find and click all five number threes!
                    </h2>
                    ...
                    <div className="bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 rounded-2xl p-6 max-w-2xl mx-auto">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-lg text-indigo-500">
                          You've learned to write and find number three!
                        </p>
                        <div className="flex gap-2 text-2xl mt-2">
                          <span className="animate-bounce delay-100">3</span>
                          <span className="animate-bounce delay-200">✨</span>
                          <span className="animate-bounce delay-300">3</span>
                          <span className="animate-bounce delay-400">✨</span>
                          <span className="animate-bounce delay-500">3</span>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default NumberThreeWorksheet; 