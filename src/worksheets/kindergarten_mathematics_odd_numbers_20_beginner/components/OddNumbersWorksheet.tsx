...
const SUCCESS_MESSAGES = [
  "Great job! That's an odd number!",
  "Perfect! You found an odd number!",
  "Excellent! Keep finding those odd numbers!",
  "Amazing! You're getting good at this!",
  "Wonderful! That number can't be divided by 2!",
];

const OddNumbersWorksheet: React.FC = () => {
  const [grid, setGrid] = useState<NumberTile[]>(createNumberGrid());
  const [message, setMessage] = useState("Click on all the odd numbers!");
  ...
  const handleTileClick = (
    ...
    if (tile.isOdd) {
      ...
      markIncorrect();
      playSound(false);
      const message = "Try again! Look for numbers that can't be divided by 2.";
      setMessage(message);
      speak(message);
    }
  };
