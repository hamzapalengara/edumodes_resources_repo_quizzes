...
const WORD_LIST = [
  // -ap words
  { word: 'CAP', pattern: '-ap', hint: 'A bottle cap', emoji: '🍶' },
  { word: 'TAP', pattern: '-ap', hint: 'Water tap', emoji: '🚰' },
  { word: 'LAP', pattern: '-ap', hint: 'Where we keep a plate while eating', emoji: '🍽️' },
  
  // -ot words
  { word: 'POT', pattern: '-ot', hint: 'Cooking pot', emoji: '🥘' },
  { word: 'HOT', pattern: '-ot', hint: 'Hot food', emoji: '🔥' },
  { word: 'DOT', pattern: '-ot', hint: 'Chocolate chip or candy dot', emoji: '🍫' },
  
  // -un words
  { word: 'BUN', pattern: '-un', hint: 'Bread bun', emoji: '🥖' },
  { word: 'FUN', pattern: '-un', hint: 'Having fun eating', emoji: '😊' },
  { word: 'SUN', pattern: '-un', hint: 'Helps fruits grow', emoji: '☀️' }
];
...
const CozyRhymingWorksheet: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasPlayedIntro && words.length > 0) {
      speak("Welcome to our cozy reading corner! Let's practice spelling rhyming words together. Click the speaker to hear each word.");
      ...
      if (isWordCorrect) {
        ...
        setTimeout(() => {
          ...
          if (currentWordIndex < words.length - 1) {
            ...
            setIsComplete(true);
            speak("Amazing! You've completed all the rhyming words!");
          }
        }, 2000);
      } else {
        markIncorrect();
...
export default CozyRhymingWorksheet; 